package cn.ydw.www.toolslib.widget.nestlistview;
...
public class NestFullListView extends LinearLayout {
    ...
    private OnItemLongClickListener mOnItemLongClickListener;// 子项长点击事件
    ...
    /**
     * 设置点击事件, item必须使用linearLayout , 不然无法使用行点击
     *
     * @param listener 行单击监听
     */
        ...
        if (null != mAdapter) {
            if (null != mAdapter.getDatas() && !mAdapter.getDatas().isEmpty()) {
                ...
                for (int i = 0; i < mAdapter.getDatas().size(); i++) {
                    final NestFullViewHolder holder;
                    ...
                    holder.getConvertView().setOnLongClickListener(new OnLongClickListener() {
                        ...
                        public boolean onLongClick(View v) {
                            if (mOnItemLongClickListener != null && mAdapter != null){
                                mOnItemLongClickListener.onItemClick(NestFullListView.this, v, mPosition);
                                return true;
                            }
                            return false;
                        }
    ...
    private List<View> mOneFooterViews;//暂时用存View，觉得可以不存。还没考虑好
     ...
     * @param footer 视图
     */
    public void addOneFooterView(View footer){
        if (null == mOneFooterViews){
            mOneFooterViews = new ArrayList<>();
        }
        for (View v: mOneFooterViews){
            removeView(v);
        }
        mOneFooterViews.clear();
        mOneFooterViews.add(footer);
        addView(footer);
    }
