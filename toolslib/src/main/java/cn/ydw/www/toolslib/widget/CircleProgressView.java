...
public class CircleProgressView extends ProgressBar {
    ...
    private int mReachBarSize = ScreenUtils.dip2px(getContext(), 2); // 未完成进度条大小
    private int mNormalBarSize = ScreenUtils.dip2px(getContext(), 2); // 未完成进度条大小
    ...
    private int mRadius = ScreenUtils.dip2px(getContext(), 20); // 半径
    private int mStartArc; // 起始角度
    private int mInnerBackgroundColor; // 内部背景填充颜色
    private int mProgressStyle = ProgressStyle.NORMAL; // 进度风格
    private int mInnerPadding = ScreenUtils.dip2px(getContext(), 1); // 内部圆与外部圆间距
    ...
    private int mOuterSize = ScreenUtils.dip2px(getContext(), 1); // 外层圆环宽度
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = ScreenUtils.dip2px(getContext(), reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = ScreenUtils.dip2px(getContext(), normalBarSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = ScreenUtils.dip2px(getContext(), radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = ScreenUtils.dip2px(getContext(), innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = ScreenUtils.dip2px(getContext(), outerSize);
        invalidate();
    }
