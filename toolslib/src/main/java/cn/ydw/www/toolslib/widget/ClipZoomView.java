...
public class ClipZoomView extends ImageView {
     ...
     */
    private final float minRate = 5/4f;
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            if (rate > minRate) { //若更高
                w = width;
                h = Math.round(width * minRate);
            } else if (rate < 1/minRate) { //若更宽
                w = Math.round(height*minRate);
                h = height;
            } else {
                w = width;
                h = height;
            ...
            //原图大小
            RectF tempSrc = MathUtils.rectFTake(0, 0, w, h);
            //控件大小
    ...
    public void setMinScale(float minScale) {
        //若设置的限制级比最大的还大, 那么修改为最大的, 比最小值小的已经处理了,
        this.minScale = minScale > MAX_SCALE? MAX_SCALE: minScale;
    }
    ...
    public boolean canScrollHorizontally(int direction) {
        if (mPinchMode == ClipZoomView.PINCH_MODE_SCALE) {
            return true;
        }
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(ClipZoomView pinchImageView);
    }
     ...
     * @see #getMaxScale()
     */
    protected float calculateNextScale(float innerScale, float outerScale, float maxScale) {
    ...
    public ClipZoomView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(ClipZoomView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(ClipZoomView.this);
            }
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mOnClickListener != null) {
                mOnClickListener.onClick(ClipZoomView.this);
            }
            return true;
        }
    });
     ...
     * @see #getMaxScale()
     * @see #scaleTab(float, float, float)
     */
    private void doubleTap(float x, float y) {
        scaleTab(x, y, getMaxScale()/2, false);
    }

    /**
     * 缩放控件到充满控件
     * @param maxScale 要放大的倍数
     *
     * @see #scaleTab(float, float, float)
     */
    public void scaleView(float maxScale) {
        float x = getWidth() /2f;
        float y = getHeight() /2f;
        scaleTab(x, y, maxScale, false);
    }
     ...
     * @param maxScale 要放大的倍数
     */
    public void nextFullScale(float maxScale){
        float x = getWidth() /2f;
        float y = getHeight() /2f;
        scaleTab(x, y, maxScale, true);
    }
     ...
     * @param x 要缩放中心点的坐标
     * @param y 要缩放中心点的坐标
     * @param maxScale 最大放大大小
     */
    private void scaleTab(float x, float y, float maxScale, boolean isOnlyFull){
        if (!isReady()) {
            return;
        }
        // TODO: 2017/12/31
        ...
        //当前总的缩放比例
        float innerScale = MathUtils.getMatrixScale(innerMatrix)[Matrix.MSCALE_X];
        float outerScale = MathUtils.getMatrixScale(mOuterMatrix)[Matrix.MSCALE_X];
        float currentScale = innerScale * outerScale;
        //重置最小限制, 如果minScale大于最小
        innerScale = Math.max(minScale, innerScale);
        //控件大小
        float displayWidth = getWidth();
        float displayHeight = getHeight();
        //接下来要放大的大小, 如果仅显示放大, 那么数值只传递限制级最大
        float nextScale = isOnlyFull? maxScale: calculateNextScale(innerScale, outerScale, maxScale);
    ...
    private void scaleEnd() {
        ...
        //整体缩放比例
        float currentScale = MathUtils.getMatrixScale(currentMatrix)[Matrix.MSCALE_X];
        //第二层缩放比例
        float outerScale = MathUtils.getMatrixScale(mOuterMatrix)[Matrix.MSCALE_X];
        //获取第一层变换矩阵
        Matrix innerMatrix = MathUtils.matrixTake();
        ...
        // TODO: 2018/1/11 手指缩放结束
        //获取最小限制级的比例
        float mInnerMax = Math.max(minScale, innerScale);
        //若整体缩放小于限制级缩放, 包含最小状态但不小于最小状态
        if (currentScale < mInnerMax) {
            scalePost = mInnerMax/innerScale/outerScale;
        }
    ...
    public static class MathUtils {
        ...
        public static float[] getMatrixScale(Matrix matrix) {
            if (matrix != null) {
                ...
                return new float[]{value[Matrix.MSCALE_X], value[Matrix.MSCALE_Y]};
            } else {
                return new float[2];
            }
        }
         ...
         */
        public static void calculateScaledRectInContainer(
                RectF container, float srcWidth, float srcHeight, ScaleType scaleType, RectF result) {
